...
import { ChainId, CurrencyAmount, ETHER, Pair, Percent, Route, Router, SmartRouter, Token, TokenAmount, Trade, WETH } from '../src'
...
describe('Router', () => {
  ...
  const pair_0_1 = new Pair(new TokenAmount(token0, JSBI.BigInt(1000)), new TokenAmount(token1, JSBI.BigInt(1000)), SmartRouter.APE)

  const pair_weth_0 = new Pair(new TokenAmount(WETH[ChainId.MAINNET], '1000'), new TokenAmount(token0, '1000'), SmartRouter.APE)
